...
#define _mav_rpm_put_float(buf, wire_offset, b)    byte_copy(&buf[wire_offset], (const char *)&b)
...
static inline void byte_copy(char *dst, const char *src)
{
	dst[0] = src[0];
	dst[1] = src[1];
	dst[2] = src[2];
...
/*  char buf[32];
	...
	_mav_put_float(buf, 16,0.1);
	_mav_put_float(buf, 20,0.1);
	_mav_put_float(buf, 24,0.1);
	_mav_put_float(buf, 28,0.1);
